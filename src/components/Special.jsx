    ...
    <section className="py-24 bg-[#0a0a0a] relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col-reverse md:flex-row justify-between items-end  mb-12 gap-4">
          <Link
            to={"/blog"}
            className="bg-gradient w-30 h-12 rounded-xl text-white flex items-center justify-center group hover:-translate-y-0.5 cursor-pointer duration-300 transition-all"
          >
            {" "}
            <FontAwesomeIcon
              icon={faAngleLeft}
        ...
        <div className="flex flex-col items-center gap-10">
          ...
          {posts.posts?.slice(0, 3).map((item, index) => (
            <Link key={index} to={`/blog/${item.slug}`}>
              <div
                dir="rtl"
                className=" grid grid-cols-1 group overflow-hidden md:grid-cols-2 gap-0 rounded-2xl border border-[#262626] hover:border-orange-500/30"
              >
                {/* image */}
                <div className="overflow-hidden relative">
                  <span className="absolute top-4 z-50 right-4 inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-orange-500 to-yellow-500 text-white text-xs font-semibold rounded-full">
                    {" "}
                  ...
                  <img
                    src={item.image}
                    alt=""
                    className=" h-72 md:h-[400px] w-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out"
                  />
                </div>
                ...
                <div className="bg-[#161616] p-8 flex flex-col justify-between">
                  <div>
                    <div className="flex gap-4 items-center mb-4">
                      <div className="flex">
                        <span className=" flex items-center justify-center px-3 py-1 border text-main border-orange-500/20 bg-[#24160B] rounded-full text-xs font-semibold bg-orange-500/10 ">
                          {item.category}
                        </span>
                      </div>
