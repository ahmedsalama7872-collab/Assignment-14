...
export default function ArticleContent({ post }) {
  let sectionsTitle = post.content
    .split("\n\n")
    .filter((line) => line.startsWith("## "));
  let paragraphs = post.content
    .split("\n\n")
    .filter((line) => !line.startsWith("## "));
      ...
      <div className="grid lg:grid-cols-[300px_1fr] gap-12">
              ...
              <nav className="flex-col flex">
                {sectionsTitle.map((el, index) => {
                  return (
                    <a
                      key={index}
                      href={`/blog/${post.slug}#section-${index}`}
        ...
        <div className="">
          <div className="p-6 bg-gradient-to-r from-orange-500/10 to-yellow-500/5 rounded-2xl border border-orange-500/20 mb-10">
            <p className="text-lg text-neutral-200 leading-relaxed text-end">
              {post.excerpt}
            </p>
          </div>
          <p className="text-neutral-300 leading-relaxed mb-6 text-lg text-end">
            {post.content.split("\n\n")[0]}
          </p>
