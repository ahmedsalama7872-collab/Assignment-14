    ...
    <>
      ...
      <div className='bg-[#0a0a0a]/95 z-[99999] backdrop-blur-xl border-b border-[#262626] fixed top-0 items-center flex left-0 right-0 h-20'>
        <div className="flex justify-between items-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          ...
          >
            <FontAwesomeIcon icon={isOpen ? faXmark : faBars} className="fa-lg" />
          </div>
      ...
      <div 
        className={`fixed top-20 left-2 right-2 bg-[#161616]/95 backdrop-blur-xl border-b rounded-b-2xl border-[#262626] z-[99998] md:hidden transition-all duration-300 ease-in-out overflow-hidden ${
          isOpen ? "max-h-[300px] opacity-100 py-6" : "max-h-0 opacity-0 py-0 border-b-0"
        }`}
        dir="rtl"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col gap-0">
          ...
          <NavLink 
            ...
            className={({ isActive }) => `w-full rounded-xl h-10 flex items-center px-6 text-sm transition-all cursor-pointer ${isActive ? "bg-orange-500/15 border border-orange-500/50 text-orange-500 font-medium" : "text-neutral-300 hover:bg-[#1a1a1a]"}`}
          >
            الرئيسية
          </NavLink>
          ...
          <NavLink 
            to="/about"
            onClick={() => setIsOpen(false)}
            className={({ isActive }) => `w-full rounded-xl h-10 text-sm flex items-center px-6 transition-all cursor-pointer ${isActive ? "bg-orange-500/15 border border-orange-500/50 text-orange-500 font-medium" : "text-neutral-300 hover:bg-[#1a1a1a]"}`}
          >
            من نحن
          </NavLink>
